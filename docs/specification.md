...
- called directly or through `defer`, and, when that call's own owner function has a body to build a CFG from, verified reachable on *every* path from that function's own entry to its exit (`CalledOnAllPaths`, computed the same `model.CFG.ReachableAvoiding` way `LL1003`'s own join-before-owner-return check is below) — a call reachable on only some paths (e.g. under `if flag`, with no other call anywhere else) is a genuine finding, not silently credited as full discharge, though a `defer` immediately after construction is still recognized as covering every path by construction (`docs/limitations.md`);
- returned;
- passed as an argument to a resolvable same-package function whose own body is verified to consume it in turn (a direct call, `defer`, or any of the other discharge forms on this list) — verified via an interprocedural fixed point (`computeParameterConsumption`) to any number of hops, regardless of declaration order; see `docs/limitations.md`, or, when the callee cannot be resolved with confidence, unconditionally as before;
- assigned to a named value;
- stored into a named field of a *keyed* struct literal assigned to a single local variable, or built directly in a `return` statement, and verified against that specific field's later use: a call through the field (same-function "stored struct" shape) or a direct, resolvable, same-package caller's own use of the field (the "constructor" shape, itself a fixed point over any number of pass-through callers, the same as direct parameter passing above) — and, for either shape, a call to a same-package method on the handle, or the handle passed on as a direct argument to a further same-package function, is itself followed into that method's or function's own body (to the same bound as a method-call/argument-passing chain elsewhere in this list), crediting whatever it consumes on the handle's behalf back to the original call site; either shape's own consuming call is also verified reachable on every path through the consuming function's own body, the identical `CalledOnAllPaths` question as the direct case just above, merged across every caller checked when there is more than one; any other, more indirect storage (an unkeyed literal, a multi-name assignment, a slice/map/array, an unresolvable callee, or a callee's own parameter this narrower check cannot name — an unnamed parameter, a `...` spread, or a variable reassigned more than once) remains unconditionally discharged, as before this capability existed.
...
- The same field/constructor ownership tracking described above for cancellation applies to a group: stored into a named field of a keyed struct literal (a single local variable, or built directly in a `return`), the obligation is verified against that field's own later `Add`/`Go`/`Wait` use — same-function, a same-package method or function call reached from the field, or, for a constructor, a fixed point over any number of pass-through callers — rather than unconditionally discharged; any more indirect storage remains unconditionally discharged, as before.

As of Phase 6 (`docs/cfg-migration-plan.md`), discharge is no longer a single flat "was `Wait` ever observed" boolean, and `Add`/`Done` counts are no longer purely qualitative — four independently-verified upgrades, each only ever firing from positive evidence:
...
- **Unjoined round** (`computeGroupRoundBalances`): reusing one `WaitGroup` for a second round of work with no matching second `Wait()` — `wg.Add(1); go worker(&wg); wg.Wait(); wg.Add(1); go worker(&wg); return` — is a genuine bug the other three upgrades do not catch: `Joined` is satisfied by the *first* `Wait()` found anywhere in the function, and count balance sums `Add`/`Done` across the whole function regardless of any `Wait()` calls in between, so both see this as fine. The function's own top-level statement sequence is split into segments at each bare, top-level `Wait()` call; the segment after the group's *last* `Wait()` has, by construction, no `Wait()` of its own following it, so any recognized worker-start there is flagged (`model.JoinGroup.UnjoinedRound`, a distinct field from `CountMismatch`, since the bug is the missing synchronization point rather than an imbalance in what it synchronizes) regardless of whether that round's own `Add`/`Done` happen to balance. Bounded to the function's own top-level, straight-line sequence, matching the other CFG-free checks in this list: a `Wait()` reached only through a branch or a loop is not treated as a boundary, and a `defer Wait()` is never treated as one either, since it runs at function exit rather than inline.
